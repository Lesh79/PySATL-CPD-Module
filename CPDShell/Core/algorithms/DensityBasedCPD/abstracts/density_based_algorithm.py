...
class DensityBasedAlgorithm(Algorithm):
    ...
    def _kernel_density_estimation(observation, bandwidth):
        kde = KernelDensity(
            kernel="gaussian",
            bandwidth=bandwidth).fit(observation)
        return kde
    ...
    def _calculate_weights(
        ...
        test_density = DensityBasedAlgorithm._kernel_density_estimation(
            test_value,
            bandwidth
            )
        reference_density = DensityBasedAlgorithm._kernel_density_estimation(
            reverence_value,
            bandwidth
            )
        ...
        def obj(alpha):
            density_ratio = np.exp(
                test_density.score_samples(test_value)
                - reference_density.score_samples(test_value) - alpha
            )
