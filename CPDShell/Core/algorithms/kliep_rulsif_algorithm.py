...
class KliepAlgorithm(Algorithm):
    ...
    def detect(self, window: Iterable[float]) -> list[int]:
        ...
        w = calculate_weights(window, window, self.bandwidth, self.lam, lambda w, alpha: -np.mean(w) + self.lam * np.sum(alpha**2))
...
class RulsifAlgorithm(Algorithm):
    ...
    def detect(self, window: Iterable[float]) -> list[int]:
        ...
        w = calculate_weights(window, window, self.bandwidth, self.lam, lambda w, alpha: np.mean((w - 1)**2) + self.lam * np.sum(alpha**2))
